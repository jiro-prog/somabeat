...
### 発見: field embedding + 入力長の相互作用で早期EOS
- **内容:** field=30 + sys_embeds≧660トークンで早期EOS（31トークンで停止）。field=0では正常。field=30 + sys_embeds=374では正常（50トークン出力）。FieldReceptor出力のノルムはsys_embedsと一致（ratio=1.00）でスケーリングは正常
- **対処:** 保留 → 会話バッファのトークン数上限導入で回避する方針
- **保留理由:** FieldReceptorとattentionパターンの相互作用の根本調査はスコープ外。回避策で本番運用は安定する見込み
...
- **モデルロード後:** allocated=6,122MiB, reserved=14,750MiB
- **3-bit時(参考):** allocated=6,324MiB (ジャーナル過去記録より)
- **差分:** allocatedは微減（-200MiB）。codebook サイズ増(8→16レベル)だがKV圧縮効率は逆に向上している可能性。reservedはPyTorchメモリプール予約で実使用量ではない
- **評価:** 8GB以内。ただし自己改善（4候補並行評価）でのピークは未検証。睡眠サイクル時に要確認
